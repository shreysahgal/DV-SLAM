...
def main():
    data = np.loadtxt("out_07.txt", delimiter=",", unpack=False)
    vertex = utils.data_to_vertex(data)
    edge = utils.vertex_to_edge(vertex)


    closure_edge = np.loadtxt("closure_edge_07.txt", unpack=False)
    edge = np.vstack((edge, closure_edge))

    print("Calculating result")
    result_poses = gtsam_batch(vertex, edge)

    np.savetxt('gtsam_result_poses_07.txt', result_poses)

    plt.plot(result_poses[:, 9], result_poses[:, 11], 'red')
    plt.plot(vertex[:, 10], vertex[:, 12], 'blue')
    plt.show()
